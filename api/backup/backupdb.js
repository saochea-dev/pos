...
module.exports.backup = async (req, res, next) => {
  ...
  const mysqldumpPath = '/Applications/XAMPP/bin/mysqldump';
...
module.exports.restore = async (req, res, next) => {
  ...
  const mysqlPath = "/Applications/XAMPP/bin/mysql";
  if (req.file) {
    exec(
      `${mysqlPath} -u ${process.env.DB_USER} -h ${process.env.DB_HOST} ${process.env.DB_NAME} < '${req.file.path}'`,
      (err, stdout, stderr) => {
        if (err) {
